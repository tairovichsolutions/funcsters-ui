...
export default function MiniLeaderBoard() {
  ...
  return (
    <div className="w-full bg-white dark:bg-[#1C1F22] rounded-xl  dark:border-gray-800 transition-colors">
      <h2 className="text-sm   text-[#0F172A] dark:text-gray-100 mb-4 tracking-tight">
        Leaderboard
      </h2>

      
      <div className="flex border bg-[#F4F5F8] border-[#DFE0E7] rounded-lg dark:bg-[#282B2E] dark:border-[#282B2E] mb-2">
        {TABS.map((tab, index) => (
          <button
            key={tab.value}
            ...
            >
              <div className="flex items-center gap-3.5">
                ...
                <div className="w-6 flex justify-center items-center">
                  ...
                  {user.rank > 3 && (
                    <span className="w-[22px] h-[22px] flex items-center justify-center rounded-full bg-[#F3F4F6] 
                    dark:bg-[#F6F6F9] text-[#4B5563] dark:text-[#0D1A26] text-xs font-semibold">
                      {user.rank}
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  />
                  <span className="text-sm font-medium text-neutral-04 dark:text-white">
                    {user.username}
                    {user.currentUser && (
      ...
      <Link href={'/leaderboard'}>
        <button className="w-full mt-3 py-2.5 text-[#0D1A26] dark:text-[#0D1A26] border-[#DFE0E7] bg-[#F4F5F8] dark:bg-white hover:bg-[#EBEDF2]   text-[13px] font-medium rounded-lg transition-colors border dark:border-gray-700">
